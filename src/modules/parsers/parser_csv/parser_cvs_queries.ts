...
export async function createCSVTable(table_name: string) {
...
export const insertBulkData = async function (table_name: string, data: Buffer[]) {
    await global.app.modules.dbs._pgpool.db_bulk_insert(`${schema}.${table_name}`, cvs_fields, data);
};
